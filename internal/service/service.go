...
func (s *Service) Run(ctx context.Context) error {
	...
	for {
		...
		case <-pullTicker.C:
			// WS delivers user checksums; REST still polls config (and full sync when WS is down).
			if s.wsClient != nil && s.wsClient.IsConnected() && s.kernel.IsRunning() {
				s.pullViaAPIAsync(ctx)
				continue
			}
			nlog.Core().Debug("polling from API (ws unavailable or kernel not running)")
			s.pullViaAPIAsync(ctx)
...
func (s *Service) initialSetup(ctx context.Context) error {
	...
	s.lastUsersMD5 = computeUsersMD5FromSpecs(bootstrap.Users)
	...
	// Fail fast: a node whose kernel never started listens on nothing, so
	// staying alive would report healthy while serving no traffic at all.
	// Exiting lets systemd restart and surfaces the cause in the journal.
	if !s.startKernel(bootstrap.Config, bootstrap.Users) {
		return fmt.Errorf("kernel %q failed to start (protocol=%s, users=%d)",
			s.cfg.Kernel.Type, bootstrap.Config.Protocol, len(bootstrap.Users))
	}
	s.markMailboxReadyAndDrain(ctx)
	return nil
}
...
func (s *Service) drainMachineMailbox(ctx context.Context) {
	...
	if state.NeedsUsersMD5Pull {
		s.handleWSEvent(ctx, controlplane.Event{
			Type:      controlplane.EventSyncUsersMD5,
			UsersMD5:  state.ExpectedUsersMD5,
			UserCount: state.ExpectedUserCount,
		})
	}
}

type etagResetter interface {
	ResetETags()
}
...
func (s *Service) requestWSResync(ctx context.Context, reason string) {
	...
	// Force full user/config payloads; a stale 304 would leave removed users online.
	if r, ok := s.source.(etagResetter); ok {
		r.ResetETags()
	}
	s.pullViaAPIAsync(ctx)
}
...
// handleWSEvent processes checksum events received via WebSocket.
func (s *Service) handleWSEvent(ctx context.Context, event controlplane.Event) {
	if event.Type != controlplane.EventSyncUsersMD5 {
		nlog.Core().Debug("ignored ws event", "type", event.Type)
		return
	}
	if event.UsersMD5 == "" {
		return
	}
	if event.UsersMD5 == s.lastUsersMD5 {
		return
	}
	if s.nodeLog != nil {
		s.nodeLog.Info(fmt.Sprintf("users md5 changed remote=%s local=%s count=%s", event.UsersMD5, s.lastUsersMD5, event.UserCount))
	} else {
		nlog.Core().Info("users md5 changed, fetching via HTTP", "remote", event.UsersMD5, "local", s.lastUsersMD5)
	}
	if _, ok := s.source.(usersFetcher); !ok {
		s.requestWSResync(ctx, "users_fetch_unsupported")
		return
	}
	if r, ok := s.source.(etagResetter); ok {
		r.ResetETags()
	}
	s.pullUsersViaAPIAsync(ctx)
}
...
func (s *Service) pullViaAPIAsync(ctx context.Context) {
	...
	wsConnected := s.wsClient != nil && s.wsClient.IsConnected()
	...
	go func() {
		...
		if wsConnected {
			pc, ok := s.source.(configPoller)
			if !ok {
				nlog.Core().Error("config poll unsupported while ws connected")
				s.pullBackoff.onFailure()
				return
			}
			config, err := pc.PollConfig(ctx)
			if err != nil {
			...
			if snapshot.Users != nil {
				...
				result.userMD5 = computeUsersMD5FromSpecs(snapshot.Users)
			}
		}
...
func (s *Service) pullUsersViaAPIAsync(ctx context.Context) {
	if !s.usersPullActive.CompareAndSwap(false, true) {
		nlog.Core().Debug("users pull already in progress, skipping")
		return
	}

	go func() {
		defer s.usersPullActive.Store(false)

		uf, ok := s.source.(usersFetcher)
		if !ok {
			nlog.Core().Error("users fetch unsupported")
			return
		}

		users, err := uf.FetchPanelUsers(ctx)
		if err != nil {
			nlog.Core().Error("fetch users failed", "error", err)
			return
		}

		result := pullResult{
			users:   model.UserSpecsFromPanel(users),
			userMD5: panel.ComputeUsersMD5(users),
		}
...
func (s *Service) applyPullResult(ctx context.Context, result pullResult) {
	...
	if result.users != nil {
		usersChanged := result.userMD5 != s.lastUsersMD5

		if usersChanged && !configChanged {
			s.applyUserUpdate(ctx, result.users, result.userMD5)
		} else if usersChanged {
			s.updateUserState(result.users)
			s.lastUsersMD5 = result.userMD5
		}
	}
...
func (s *Service) prepareUserState(users []model.UserSpec) (prevUsers []model.UserSpec, prevHash string) {
	...
	s.metricsMu.Unlock()
	s.lastUsersMD5 = computeUsersMD5FromSpecs(users)
...
func (s *Service) applyUserUpdate(ctx context.Context, users []model.UserSpec, newMD5 string) {
	prevUsers, prevHash := s.prepareUserState(users)
	if newMD5 != "" {
		s.lastUsersMD5 = newMD5
	}

	if !s.ensureRunning() {
		if s.nodeLog != nil {
			s.nodeLog.Warn("users updated in memory; kernel not running yet")
		} else {
			nlog.Core().Warn("users updated in memory; kernel not running yet")
		}
		return
	}

	toClose := removedUserSpecs(prevUsers, users)
	s.closeUserConnections(ctx, toClose)
...
func (s *Service) applyUserDelta(ctx context.Context, action string, deltaUsers []model.UserSpec) {
	...
	case "add":
		...
		if !s.ensureRunning() {
			if s.nodeLog != nil {
				s.nodeLog.Warn("users delta add stored; kernel not running yet")
			}
			return
		}
	...
	case "remove":
		...
		wantRemoved := len(s.lastUsers) - len(filtered)

		// Always update local user state first so reports/metrics stay correct
		// even if the kernel is stopped or RemoveUsers is a no-op.
		prevUsers, prevHash := s.prepareUserState(filtered)

		if !s.kernel.IsRunning() {
			if s.nodeLog != nil && wantRemoved > 0 {
				s.nodeLog.Info(fmt.Sprintf("users removed (kernel stopped): -%d", wantRemoved))
			}
			return
		}

		s.closeUserConnections(ctx, deltaUsers)
		...
		if err != nil {
			...
			removed = wantRemoved
		} else if removed == 0 && wantRemoved > 0 {
			// Kernel user map missed the IDs (e.g. only UUID keyed); force full swap.
			nlog.Core().Warn("RemoveUsers removed 0 users, falling back to UpdateUsers")
			if _, _, err := s.kernel.UpdateUsers(filtered); err != nil {
				nlog.Core().Error(fmt.Sprintf("UpdateUsers fallback failed: %v", err))
				s.restoreUserState(prevUsers, prevHash)
				return
		...
		} else if s.nodeLog != nil && wantRemoved == 0 {
			s.nodeLog.Debug("users delta remove: targets not in local set")
		}
...
func removedUserSpecs(prev, next []model.UserSpec) []model.UserSpec {
	if len(prev) == 0 {
		return nil
	}
	keep := make(map[int]struct{}, len(next))
	for _, u := range next {
		keep[u.ID] = struct{}{}
	}
...
func (s *Service) closeUserConnections(ctx context.Context, users []model.UserSpec) {
	if len(users) == 0 || !s.kernel.IsRunning() {
		return
	}
	closed := 0
	for _, u := range users {
		...
		if err := s.kernel.CloseUserConnections(ctx, u.UUID); err != nil {
			nlog.Core().Warn("close user connections failed", "user_id", u.ID, "error", err)
			continue
		}
		closed++
	}
	if closed > 0 && s.nodeLog != nil {
		s.nodeLog.Info(fmt.Sprintf("user connections closed: %d users", closed))
	}
}
...
// computeUsersMD5FromSpecs mirrors the panel checksum algorithm for local state.
func computeUsersMD5FromSpecs(users []model.UserSpec) string {
	if users == nil {
		return panel.ComputeUsersMD5(nil)
	}
	panelUsers := make([]panel.User, len(users))
	for i, u := range users {
		panelUsers[i] = panel.User{
			ID:               u.ID,
			UUID:             u.UUID,
			SpeedLimit:       u.SpeedLimit,
			DeviceLimit:      u.DeviceLimit,
			PlanID:           u.PlanID,
			UnlimitedTraffic: u.UnlimitedTraffic,
		}
	}
	return panel.ComputeUsersMD5(panelUsers)
}

type configPoller interface {
	PollConfig(ctx context.Context) (*model.NodeSpec, error)
}

type usersFetcher interface {
	FetchPanelUsers(ctx context.Context) ([]panel.User, error)
}
...
func (s *Service) sendDeviceBatch() {
	...
	if len(devices) == 0 {
		nlog.Core().Debug("device snapshot empty, skipping")
		return
	}
