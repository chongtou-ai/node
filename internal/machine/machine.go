...
type Orchestrator struct {
	...
	// Per-node kernel config (may differ from global after transport auto-switch).
	eventsMu    sync.RWMutex
	mailboxes   map[int]*controlplane.NodeMailbox
	nodeKernels map[int]config.KernelConfig
...
func New(cfg *config.Config) *Orchestrator {
	...
	return &Orchestrator{
		...
		nodeKernels: make(map[int]config.KernelConfig),
...
func (o *Orchestrator) startNode(ctx context.Context, mn panel.MachineNode) {
	...
	o.nodeKernels[mn.ID] = nodeCfg.Kernel
	o.eventsMu.Unlock()
...
// onWSEvent routes a WS checksum event to the correct node's mailbox.
func (o *Orchestrator) onWSEvent(event panel.WSEvent) {
	if event.Type != panel.WSEventSyncUsersMD5 {
		nlog.Core().Debug("machine ws ignored event", "type", event.Type)
		return
	}

	nodeID := event.NodeID
	if nodeID == 0 {
		nlog.Core().Warn("machine ws event missing node_id, dropping", "type", event.Type)
		return
	}
	...
	o.eventsMu.RUnlock()
	if !ok {
		nlog.Core().Warn("machine ws event for unknown node, dropping",
			"node_id", nodeID, "type", event.Type)
		return
	}

	translated, err := controlplane.TranslateWSEvent(event)
	if err != nil {
	...
	nlog.Core().Debug("machine ws checksum applied to mailbox",
		"node_id", nodeID, "md5", event.UsersMD5)
