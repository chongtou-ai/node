...
func (p *MachinePanelControlPlane) PollConfig(ctx context.Context) (*model.NodeSpec, error) {
	configSnapshot, err := p.client.GetConfig()
	if err != nil {
		return nil, fmt.Errorf("machine poll config: %w", err)
	}
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
...
func (p *MachinePanelControlPlane) FetchPanelUsers(ctx context.Context) ([]panel.User, error) {
	users, err := p.client.GetUsers()
	if err != nil {
		return nil, fmt.Errorf("machine fetch users: %w", err)
	}
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
...
func (p *MachinePanelControlPlane) ResetETags() {
	if p.client != nil {
		p.client.ResetETags()
	}
}
