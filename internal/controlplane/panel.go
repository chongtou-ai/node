...
func (p *PanelControlPlane) Initial(ctx context.Context, metricsFn func() map[string]interface{}, events chan<- Event, statuses chan<- StatusChange) (Bootstrap, error) {
	...
	if bootstrap.PushInterval == 0 && configSnapshot.BaseConfig.PushInterval > 0 {
		bootstrap.PushInterval = configSnapshot.BaseConfig.PushInterval
	}
	if bootstrap.PullInterval == 0 && configSnapshot.BaseConfig.PullInterval > 0 {
		bootstrap.PullInterval = configSnapshot.BaseConfig.PullInterval
	}
...
func (p *PanelControlPlane) PollConfig(ctx context.Context) (*model.NodeSpec, error) {
	configSnapshot, err := p.client.GetConfig()
	if err != nil {
		return nil, fmt.Errorf("poll config: %w", err)
	}
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	...
	return model.NodeSpecFromPanelValidated(configSnapshot, p.kcfg)
}

func (p *PanelControlPlane) FetchPanelUsers(ctx context.Context) ([]panel.User, error) {
	users, err := p.client.GetUsers()
	if err != nil {
...
func (p *PanelControlPlane) newPushClient(metricsFn func() map[string]interface{}, events chan<- Event, statuses chan<- StatusChange, wsURL string) PushClient {
	...
	inner := panel.NewWSClient(
		...
		func(event panel.WSEvent) {
			translated, err := TranslateWSEvent(event)
			if err != nil {
				nlog.Core().Warn("invalid ws event, dropping", "type", event.Type, "error", err)
				return
			}
			select {
...
func TranslateWSEvent(event panel.WSEvent) (Event, error) {
	return Event{
		Type:      EventType(event.Type),
		UsersMD5:  event.UsersMD5,
...
func (p *PanelControlPlane) ResetETags() {
	if p.client != nil {
		p.client.ResetETags()
	}
}
