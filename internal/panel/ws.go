...
// WSEvent types
const (
	WSEventSyncUsersMD5  = "sync.users.md5" // panel → node: user list checksum
...
type syncUsersMD5Payload struct {
	MD5       string `json:"md5"`
...
func (w *WSClient) handleMessage(msg wsMessage) {
	...
	case WSEventSyncUsersMD5:
		w.handleDataEvent(msg)

	default:
		nlog.Core().Debug("ws ignored event", "event", msg.Event)
	}
}
...
func (w *WSClient) handleDataEvent(msg wsMessage) {
	...
	switch msg.Event {
	case WSEventSyncUsersMD5:
		var p syncUsersMD5Payload
		if err := decodeData(msg.Data, &p); err != nil {
			nlog.Core().Warn("ws: cannot decode users md5 payload", "error", err)
			return
		}
		if p.MD5 == "" {
			nlog.Core().Warn("ws: users md5 payload missing md5")
			return
		}
		event.UsersMD5 = p.MD5
		event.UserCount = p.UserCount
