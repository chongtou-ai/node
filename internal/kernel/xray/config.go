...
func buildRealitySettings(nc *model.NodeSpec) M {
	...
	setRealityString(reality, nc.TLSSettings, "min_client_ver", "minClientVer")
	setRealityString(reality, nc.TLSSettings, "max_client_ver", "maxClientVer")
	if v, ok := nc.TLSSettings["max_time_diff"]; ok && v != nil {
		switch n := v.(type) {
		case int:
			reality["maxTimeDiff"] = n
		case int64:
			reality["maxTimeDiff"] = int(n)
		case float64:
			reality["maxTimeDiff"] = int(n)
		case string:
			if ms, err := strconv.Atoi(strings.TrimSpace(n)); err == nil {
				reality["maxTimeDiff"] = ms
			}
		}
	}
...
func setRealityString(reality, tlsSettings M, panelKey, xrayKey string) {
	if tlsSettings == nil {
		return
	}
	v, ok := tlsSettings[panelKey]
	if !ok || v == nil {
		return
	}
