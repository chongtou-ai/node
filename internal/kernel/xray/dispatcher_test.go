...
func TestLimitDispatcher_FirstComeRejectsEarlierIP(t *testing.T) {
	ld := newTestDispatcher()
	email := userEmail(1)
	ld.UpdateLimits(map[string]int{email: 1}, map[string]int{email: 1}, nil)

	if ld.checkDeviceLimit(email, "39.144.145.105", true) {
		t.Fatal("first IP should be allowed")
	}
	if !ld.checkDeviceLimit(email, "39.130.52.58", true) {
		t.Fatal("second IP should be rejected even when it sorts before the first")
	}
	if ld.checkDeviceLimit(email, "39.144.145.105", true) {
		t.Fatal("occupying IP should still be allowed")
	}
...
func TestLimitDispatcher_TrackLinkPreservesReader(t *testing.T) {
	...
	ld.trackLink(link, sessionRef{email: email, sourceIP: "1.1.1.1", isTCP: true})
...
func TestLimitDispatcher_CloseUserConnections(t *testing.T) {
	ld := newTestDispatcher()
	email := userEmail(1)
	ld.UpdateLimits(map[string]int{email: 1}, map[string]int{email: 1}, nil)

	link1 := &transport.Link{Reader: nopReader{}, Writer: buf.Discard}
	link2 := &transport.Link{Reader: nopReader{}, Writer: buf.Discard}
	ld.trackLink(link1, sessionRef{email: email, sourceIP: "1.1.1.1", isTCP: true})
	ld.trackLink(link2, sessionRef{email: email, sourceIP: "1.1.1.1", isTCP: true})

	if got := ld.connCount.Load(); got != 2 {
		t.Fatalf("expected connCount=2, got %d", got)
	}
	if closed := ld.CloseUserConnections(email); closed != 2 {
		t.Fatalf("CloseUserConnections() = %d, want 2", closed)
	}
	if got := ld.connCount.Load(); got != 0 {
		t.Fatalf("expected connCount=0 after force close, got %d", got)
	}
}

// fakeConn records whether the client connection was torn down.
...
// An expired user mid-splice is only really kicked once the client conn dies,
// since splice and XTLS direct copy never touch the tracked writer.
func TestLimitDispatcher_CloseUserConnectionsDropsClientConn(t *testing.T) {
	ld := newTestDispatcher()
	email := userEmail(1)
	ld.UpdateLimits(map[string]int{email: 1}, map[string]int{email: 1}, nil)
	...
	link1 := &transport.Link{Reader: nopReader{}, Writer: buf.Discard}
	link2 := &transport.Link{Reader: nopReader{}, Writer: buf.Discard}
	// Both links ride the same client conn, as mux/XUDP sub-connections do.
	ld.trackLink(link1, sessionRef{email: email, sourceIP: "1.1.1.1", isTCP: true, conn: shared})
	ld.trackLink(link2, sessionRef{email: email, sourceIP: "1.1.1.1", isTCP: true, conn: shared})

	ld.CloseUserConnections(email)

	if got := shared.closes.Load(); got != 1 {
		t.Fatalf("client conn should be closed exactly once, got %d", got)
	}
}

func TestLimitDispatcher_CloseUserConnectionsHandlesNilConn(t *testing.T) {
	ld := newTestDispatcher()
	email := userEmail(1)
	ld.UpdateLimits(map[string]int{email: 1}, map[string]int{email: 1}, nil)

	// UDP inbounds leave session.Inbound.Conn nil.
	link := &transport.Link{Reader: nopReader{}, Writer: buf.Discard}
	ld.trackLink(link, sessionRef{email: email, sourceIP: "1.1.1.1", isTCP: false})

	if closed := ld.CloseUserConnections(email); closed != 1 {
		t.Fatalf("CloseUserConnections() = %d, want 1", closed)
	}
}
