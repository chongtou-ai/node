...
func TestBuildConfig_VLESS_RealityMinClientVer(t *testing.T) {
	nc := panel.NodeConfig{
		Protocol:   "vless",
		ServerPort: 443,
		TLS:        2,
	...
	if reality["minClientVer"] != "0.0.0" {
		t.Errorf("minClientVer = %v, want 0.0.0", reality["minClientVer"])
	}
	if reality["maxClientVer"] != "26.7.11" {
		t.Errorf("maxClientVer = %v, want 26.7.11", reality["maxClientVer"])
	}
	if reality["maxTimeDiff"] != float64(60000) && reality["maxTimeDiff"] != 60000 {
		t.Errorf("maxTimeDiff = %v, want 60000", reality["maxTimeDiff"])
	}
}
