...
type ConnTracker struct {
	...
	connMap       map[string]net.Conn // connID → TCP conn (force-close support)
	packetConnMap map[string]N.PacketConn
	connUUID      map[string]string // connID → user UUID
...
func NewConnTracker(_ int) *ConnTracker {
	return &ConnTracker{
		...
		packetConnMap: make(map[string]N.PacketConn),
...
func (t *ConnTracker) RoutedConnection(
	...
	// Device limit gate-keeping (reserves the IP when allowed).
	reserved := false
	if dlf := t.deviceLimitFunc.Load(); dlf != nil {
		if limit, hasLimit := (*dlf)(uuid); hasLimit {
			reserved = true
			if t.admitDevice(us, uid, sourceIP, limit) {
...
// checkDeviceGate reports whether a new source IP should be rejected.
// First-come-first-served: already-seen IPs stay allowed; a new IP is
// rejected once local (+ fresh global) distinct IPs have reached limit.
func (t *ConnTracker) checkDeviceGate(us *userStats, userID int, sourceIP string, limit int) bool {
	return t.gateDevice(us, userID, sourceIP, limit, false)
}

// admitDevice is the connection-time gate: same rule as checkDeviceGate,
// but reserves the IP under the user lock so two new IPs cannot race in.
func (t *ConnTracker) admitDevice(us *userStats, userID int, sourceIP string, limit int) bool {
	return t.gateDevice(us, userID, sourceIP, limit, true)
}

func (t *ConnTracker) gateDevice(us *userStats, userID int, sourceIP string, limit int, reserve bool) bool {
	if us == nil || limit <= 0 {
		return false
	}
	...
	if time.Since(t.globalLastUpdate) <= 60*time.Second {
	...
	nlog.Core().Debug("device limit: rejecting new IP",
		"userID", userID, "ip", sourceIP, "occupied", occupied, "limit", limit)
	return true
}
...
func (t *ConnTracker) CloseByUUID(uuid string) int {
	...
	for id, owner := range t.connUUID {
		if owner != uuid {
			continue
		}
		if c, ok := t.connMap[id]; ok {
			toClose = append(toClose, c)
		}
		if pc, ok := t.packetConnMap[id]; ok {
	...
	if closed > 0 {
		nlog.Core().Info("singbox: closed user connections", "uuid", uuid, "count", closed)
	}
	return closed
}
