...
func TestConnTrackerCheckDeviceGateMergesFreshGlobalDevices(t *testing.T) {
	...
	if !tracker.checkDeviceGate(us, 1, "2.2.2.2", 2) {
		t.Fatal("expected new IP to be rejected when local+global already occupy the limit")
	}
	if !tracker.checkDeviceGate(us, 1, "99.99.99.99", 2) {
		t.Fatal("expected merged local+global device state to reject a third device")
	}
...
func TestConnTrackerCheckDeviceGateRejectsEarlierIPWhenFull(t *testing.T) {
	tracker := NewConnTracker(0)
	tracker.SetUserMap(map[string]int{"uuid-1": 1})
	us := tracker.users[1]
	us.addConn("39.144.145.105")

	if !tracker.checkDeviceGate(us, 1, "39.130.52.58", 1) {
		t.Fatal("new IP must be rejected even when it sorts before the occupant")
	}
	if tracker.checkDeviceGate(us, 1, "39.144.145.105", 1) {
