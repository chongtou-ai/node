...
release_asset_url() {
    ...
    if [ "$RELEASE_VERSION" = "latest" ]; then
        echo "${base}/latest/download/${artifact}"
    else
        echo "${base}/download/${RELEASE_VERSION}/${artifact}"
    fi
}
...
download_release_artifact() {
    local artifact="$1"
    local dest="$2"
    local bases=("$DEFAULT_DOWNLOAD_BASE")
    if [ -n "${FALLBACK_DOWNLOAD_BASE:-}" ] && [ "$FALLBACK_DOWNLOAD_BASE" != "$DEFAULT_DOWNLOAD_BASE" ]; then
        bases+=("$FALLBACK_DOWNLOAD_BASE")
    fi
    local base url
...
stage_binary() {
    ...
    if [ -n "$local_src" ]; then
        ...
        if ! download_release_artifact "xboard-node-linux-${ARCH}" "$staged"; then
            log_error "Failed to download xboard-node-linux-${ARCH} from ${DEFAULT_DOWNLOAD_BASE} or ${FALLBACK_DOWNLOAD_BASE}"
            exit 1
        fi
    fi
    chmod +x "$staged"
...
stage_xbctl() {
    ...
    if [ -n "$local_src" ]; then
        ...
        if ! download_release_artifact "xbctl-linux-${ARCH}" "$staged"; then
            log_error "Failed to download xbctl-linux-${ARCH} from ${DEFAULT_DOWNLOAD_BASE} or ${FALLBACK_DOWNLOAD_BASE}"
            exit 1
        fi
    fi
    chmod +x "$staged"
...
Documentation=https://github.com/chongtou-ai/node
